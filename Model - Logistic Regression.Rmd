...
final_data <- final_data %>% mutate(across(everything(), ~ replace_na(., 0)))
...
# model
```{r}
exclude <- c("gamePlayId", "pff_manZone")

variables <- setdiff(names(train), exclude) 
...
pred_vars <- setdiff(names(train), exclude) 
formula <- as.formula(paste("pff_manZone ~", paste(pred_vars, collapse = " + ")))
...
plot(step_residuals) # looks exact same
...
metrics <- data.frame(
    ...
    Accuracy = sapply(thresholds, function(thresh) 
        mean(ifelse(sig_predictions2 >= thresh, 1, 0) == test$pff_manZone)))
...
coef_df$LowerCI <- coef_df$Estimate - 1.96 * coef_df$SE
coef_df$UpperCI <- coef_df$Estimate + 1.96 * coef_df$SE

ggplot(coef_df, aes(x = Estimate, y = Variable)) +
    geom_point() +
