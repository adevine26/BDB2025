...
```{r, include=FALSE}
games <- fread("games.csv", data.table=FALSE)
plays <- fread("plays.csv", data.table=FALSE)
players <- fread("players.csv", data.table=FALSE)
player_stats <- fread("player_play.csv", data.table=FALSE)

track_season <- data.table()
for(i in seq(1,9)) {
    temp <- fread(paste0("tracking_week_", i, ".csv"))
...
offensive_actions_summary <- ballsnap  %>%
    ...
    summarise(
        any_inMotionAtBallSnap = any(inMotionAtBallSnap == TRUE, na.rm = TRUE),
...
responses <- off_motion %>%
    ...
    summarise(
        closest_defender_start = nflId.y[which.min(distance)], 
        .groups = "drop"
    )
